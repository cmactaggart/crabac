...
export function RouteBuilderModal({ onClose, onSave, defaultVisibility = 'private' }: Props) {
  ...
  useEffect(() => {
    ...
    waypoints.forEach((wp, i) => {
      ...
      el.style.width = isFirst || isLast ? '18px' : '14px';
      el.style.height = isFirst || isLast ? '18px' : '14px';
  ...
  }, [waypoints, geojson]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    <div style={styles.overlay}>
      <div style={styles.modal}>
        ...
        <div style={styles.header}>
          ...
          <button onClick={onClose} style={styles.toolBtn} title="Close">
            <X size={18} />
          </button>
        </div>
        ...
        {waypoints.length === 1 && (
          <div style={styles.hint}>
            Click to add your next waypoint — the route will snap to roads
          </div>
        )}
        ...
        {showSavePanel && (
          <div style={styles.savePanelOverlay} onClick={() => setShowSavePanel(false)}>
            <div style={styles.savePanel} onClick={(e) => e.stopPropagation()}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <div style={{ flex: 1 }}>
                  ...
                  <select value={visibility} onChange={(e) => setVisibility(e.target.value as PersonalVisibility)} style={styles.input}>
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
                <button
                  ...
                  style={{ ...styles.primaryBtn, opacity: saving || !routeName.trim() ? 0.6 : 1 }}
                >
                  {saving ? 'Saving...' : 'Save Route'}
                </button>
                <button onClick={() => setShowSavePanel(false)} style={styles.cancelBtn}>
                  Cancel
                </button>
              </div>
