...
interface ChannelsState {
  ...
  createChannel: (spaceId: string, name: string, displayName?: string, topic?: string, categoryId?: string, type?: string, isPrivate?: boolean, memberIds?: string[], roleOverrides?: string[]) => Promise<Channel>;
...
export const useChannelsStore = create<ChannelsState>((set, get) => ({
  ...
  createChannel: async (spaceId, name, displayName, topic, categoryId, type, isPrivate, memberIds, roleOverrides) => {
    const channel = await api<Channel>(`/spaces/${spaceId}/channels`, {
      method: 'POST',
      body: JSON.stringify({ name, displayName, topic, categoryId, type, isPrivate, memberIds, roleOverrides }),
    });
